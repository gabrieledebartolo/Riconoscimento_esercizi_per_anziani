...
def draw_result_img(img_disp, ith_img, humans, dict_id2skeleton,
    ...
    #drawing blank space right
    img_disp = lib_plot.add_white_region_to_right_of_image(img_disp)
    #print second in display

    td = timedelta(seconds=(ith_img/DST_VIDEO_FPS ))
    
    chop_microseconds(td)
   
    cv2.putText(img_disp, "Time:" + str(td).split(".")[0],
...
# Loads an image
img = cv2.imread("C:/Users/Gabriele/Desktop/gatto.jpg")
...
img = cv2.copyMakeBorder(img, top, 0,0,0, borderType, None, value)

cv2.putText(img, text="GATTO", org=(int(img.shape[1]*2/5), 80),
