...
def get_id_classes(data, idx_rows):
    id_classes = []
    for idx_row in idx_rows:
        row_class = data[idx_row][3]
...
def overlap(data):
    ...
    while(idx_row < len(data)):
        ...
        matrix, offset = get_matrix(data, idx_row)
        for row in matrix:
            data_overlap.append(row)
        idx_row = int(idx_row + offset)
        print("rows done: " + str(idx_row) + " (" + str(int(idx_row*100/len(data))) + "%)")
    data_y = get_id_classes(data, idx_rows)
    return data_overlap, data_y
...
def main():
    ...
    testX_overlap, testy = overlap(test_data)
    with open(DST_TEST_SET_OVERLAP, 'w',newline='') as csv_X:
        writer = csv.writer(csv_X)
        for row in testX_overlap:
            writer.writerow(row)
