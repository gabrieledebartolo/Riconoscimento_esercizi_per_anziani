...
class ClassifierOfflineTrain(object):
    ...
    def train(self, X, Y):
        ...
        print("Sum eig values auto:", np.sum(self.pca.explained_variance_ratio_))
    ...
    def _init_all_models(self):
        ...
        self.classifiers = [
            ...
            MLPClassifier((100, 100, 100)),  # Neural Net
            AdaBoostClassifier(),
            GaussianNB(),
    ...
    def draw_scores_onto_image(self, img_disp):
        ...
        for i in range(-1, len(self.action_labels)):
            ...
            if i == -1:
                ...
                #color_g *= (0.0 + 1.0 * self.scores[i])**0.5
            ...
            cv2.putText(img_disp, text=s, org=(TXT_X, TXT_Y),
                       fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=FONT_SIZE,
                        color=(0, 0, color_g), thickness=2) #
  
            '''
            if self.scores.max() > self.THRESHOLD_SCORE_FOR_DISP:  # If lower than threshold, bad
            # print text good
                cv2.putText(img_disp, textg, org=(TX_X, TX_Y), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=FONT_SIZE_G,  
                             color=(0,int(color_g),0), thickness=2)
            else:
                cv2.putText(img_disp, texti, org=(TX_X, TX_Y), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=FONT_SIZE_G,  
